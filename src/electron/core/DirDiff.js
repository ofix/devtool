...
export class DirDiff {
  ...
  buildSymmetricTrees (treeA, treeB, flatDiff, rootA, rootB) {
    const buildSymmetricNode = (nodeA, nodeB, currentRelPath = '.') => {
      // 空节点填充
      const symmetricNode = {
        left: nodeA || { ...EMPTY_NODE, relativePath: currentRelPath },
        right: nodeB || { ...EMPTY_NODE, relativePath: currentRelPath },
        children: []
      };

      // 标记差异状态（对齐renderer的diffType）
      if (nodeA && !nodeB) {
      ...
      Array.from(allChildNames).forEach(childName => {
        ...
        const childRelPath = currentRelPath === '.'
          ? childName
          : path.join(currentRelPath, childName);
