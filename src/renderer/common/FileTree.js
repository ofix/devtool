import { FileNodeType, SortDirection, SortField } from "./FileNodeType.js";
import FileTreeNode from "./FileTreeNode.js";

class FileTree {
    /**
     * @param {Object} [options] - 配置选项
     * @param {string} [options.rootPath='/'] - 初始根目录路径
     * @param {string} [options.rootName='.'] - 初始根节点显示名称
     * @param {Object} [options.sortConfig] - 排序配置
     * @param {boolean} [options.sortConfig.enabled=true] - 是否启用排序（默认关闭）
     * @param {SortField} [options.sortConfig.field=SortField.NAME] - 排序字段
     * @param {SortDirection} [options.sortConfig.direction=SortDirection.ASC] - 排序方向
     * @param {boolean} [options.enablePathCache=true] - 是否启用路径查找缓存（平衡性能/内存）
     */
    constructor(options = {}) {
        // 基础配置
        this.config = {
            rootPath: options.path || "/",
            rootName: options.name || "root",
        };

        // 排序配置
        this.sortConfig = {
            enabled: options.sortConfig?.enabled ?? true,
            field: options.sortConfig?.field ?? SortField.NAME,
            direction: options.sortConfig?.direction ?? SortDirection.ASC,
        };

        // 路径查找缓存（LRU 策略，默认启用）
        this.enablePathCache = options.enablePathCache ?? true;
        this.pathCache = new Map(); // 缓存格式：key=path, value=FileTreeNode
        ...
        this.adapter = null;
        this.setRootNode(options.name, options.path, options.size, options.mtime);
    }
    ...
    setRootNode(nodeName, absPath, size, mtime) {
    ...
    findNodeByPath(targetPath, startNode = this.root) {
        ...
        // 递归终止条件：找到目标节点
        if (startNode.path === normalizedPath) {
            // 更新缓存
            if (this.enablePathCache) {
        ...
        const currentPathParts = startNode.path.split("/").filter(part => part);
    ...
    addChildren(parentNodePath, children) {
        ...
        children.forEach(child => {
            // 补全子项完整路径
            const childFullPath = child.path || (normalizedParentPath === "/"
                ? `/${child.name}`
    ...
    build(dirs, files) {
        ...
        files.forEach(file => {
            ...
            // 检查文件是否已存在
            if (this.findNodeByPath(file.path)) return;
            ...
            // 更新缓存（若启用）
            if (this.enablePathCache) {
                this.updatePathCache(file.path, fileNode);
            }
        });

        // 全局排序
        if (this.sortConfig.enabled) {
            this.root.sortRecursively(this.sortConfig);
        }
    }
    ...
    changeDirectory(targetPath) {
        ...
        console.log(`已导航到：${this.currentRoot.path}`);
        return true;
    }

    // 返回上层目录
    navigateUp() {
        if (this.currentRoot.path === "/") {
        ...
        if (parentNode && parentNode.isDirectory()) {
            ...
            console.log(`已向上导航到：${this.currentRoot.path}`);
            return true;
        }
    ...
    formatTree(node, prefix = "", isLast = true, isSibling = false) {
        ...
        const branch = isLast ? "└──" : "├──";
        const rootMarker = !isSibling && node.path === this.currentRoot.path ? "" : "";
        ...
        if (node.isDirectory() && node.children?.length) {
            const shouldExpand = !isSibling || node.path === this.currentRoot.path;
            if (shouldExpand) {
     ...
     */
    getFormattedTree() {
        const currentPath = this.currentRoot.path;
        const sortFieldMap = {
            [SortField.NAME]: "名称",
            [SortField.SIZE]: "大小",
            [SortField.DATE]: "日期",
        ...
        if (!parentNode) {
            ...
            allSameLevelNodes.forEach((node, index) => {
                ...
                const isSibling = node.path !== this.currentRoot.path;
    ...
    /**
     * 递归折叠单子目录节点
     * @param {Object} node - 当前节点（已拷贝）
     * @param {Array} segmentChain - 当前路径链（用于记录折叠路径）
     * @returns {Object} 折叠后的节点
     */
    collapseNode(node, segmentChain = []) {
        // 如果是文件，直接返回
        ...
        if (children.length === 1 && children[0].type === FileNodeType.DIRECTORY) {
            ...
            // 递归处理子节点
            const foldedChild = this.collapseNode(child, currentChain);

            // 创建折叠节点
            return {
                name: node.name,
                path: node.path,
                type: FileNodeType.COLLAPSE_DIR,    // 标记为折叠节点
                // 存储路径片段（用于渲染）
                segments: currentChain.map(n => ({
                    name: n.name,
                    path: n.path
                })),
                loaded: node.loaded,
