...
const switchTab = (index) => {
  ...
  // 切换后将当前Tab滚动到可视区域
  const tabItems = tabListRef.value?.querySelectorAll(".tab-item");
  if (tabItems && tabItems[index]) {
    tabItems[index].scrollIntoView({ inline: "center", behavior: "smooth" });
  }
  // 切换Tab清空搜索框
  searchKeyword.value = "";
};
...
.app-help::-webkit-scrollbar-thumb:hover {
  background: rgba(59, 130, 246, 0.3);
}
.app-help::-webkit-scrollbar-track {
  background: transparent;
}
</style>
