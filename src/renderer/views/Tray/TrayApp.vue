<template>
      ...
      <div class="common-apps">
        ...
        <div class="apps-grid common-grid" v-show="commonExpanded">
          <div
            ...
            @contextmenu.prevent="!app.disabled && handleCommonAppRightClick($event, app)"
          >
      ...
      <div class="all-apps">
        ...
        <div class="apps-grid all-grid" v-show="allExpanded">
          <div
            ...
            :class="{ disabled: app.disabled }"
            @click="!app.disabled && openApp(app.key)"
            @contextmenu.prevent="!app.disabled && handleAllAppRightClick($event, app)"
          >
            <component :is="`Icon${app.icon}`" class="app-icon" />
            <span class="app-name">{{ app.name }}</span>
          </div>
  ...
  const allApps = ref([
    { key: "ScreenshotToolWnd", name: "截图", icon: "Screenshot", disabled: false },
    { key: "ScreenRulerWnd", name: "屏幕标尺", icon: "Ruler", disabled: false },
    { key: "VideoRecordWnd", name: "视频录制", icon: "VideoRecord", disabled: true }, // 示例：禁用
    {
      key: "HexEditorWnd",
      name: "十六进制编辑器",
      icon: "HexEditor",
  ...
  function updateCommonApps() {
    ...
    // 步骤1：优先显示自定义顺序的应用
    const customApps = customOrder.map(key => {
  ...
  .tray-window {
    ...
    border-radius: 8px; /* Windows 11圆角 */
    padding: 16px;
