export default class MinuteKlineRenderer {
    ...
    setData(data) {
      ...
      this.data = data;
      this.preClosePrice = data[0].preClose || data[0].price;
      this.calculateStats();
      this.calculateMACD();
    ...
    calculateStats() {
      ...
      for (let i = 0; i < this.data.length; i++) {
        ...
        maxPrice = Math.max(maxPrice, item.price, item.avgPrice || 0);
        minPrice = Math.min(minPrice, item.price, item.avgPrice || 0);
    ...
    updateViewRange() {
      const visibleCount = Math.floor(this.chartWidth / 2); // 每个数据点2px宽度
      this.viewRange.end = Math.min(this.data.length, this.viewRange.start + visibleCount);
      this.viewRange.start = Math.max(0, this.viewRange.end - visibleCount);
    }
    ...
    drawPriceLine() {
      ...
      const step = (this.chartWidth - chartLeft - chartRight) / (this.data.length - 1);
      ...
      for (let i = 0; i < visibleData.length; i++) {
        ...
        const x = chartLeft + (start + i) * step;
    ...
    drawVolume() {
      const { ctx, chartLeft, chartRight, chartTop, mainChartHeight, config } = this;
      const step = (this.chartWidth - chartLeft - chartRight) / this.data.length;
    ...
    drawMACD() {
      ...
      const { ctx, chartLeft, chartRight, chartTop, mainChartHeight, config } = this;
      const step = (this.chartWidth - chartLeft - chartRight) / this.macdData.length;
    ...
    drawAxes() {
      ...
      // 价格标签
      for (let i = 0; i <= 4; i++) {
        const price = this.minPrice + (priceRange * i) / 4;
        const y = chartTop + mainChartHeight * (1 - i / 4);
