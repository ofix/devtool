...
class SceneManager extends EventTarget {
  ...
  _bindCanvasEvents() {
    ...
    canvas.addEventListener('wheel', (e) => this._handleWheelScale(e));
  }
  ...
  _handleMouseDown(event) {
    const pos = this._getCanvasMousePos(event);
  ...
  _handleWheelScale(event) {
    ...
    // 计算新缩放与偏移
    this.scale = Math.max(0.1, Math.min(5, this.scale * scaleStep));
    const scaleDelta = this.scale / oldScale;
    this.offset.x = pos.x - (pos.x - this.offset.x) * scaleDelta;
    this.offset.y = pos.y - (pos.y - this.offset.y) * scaleDelta;

    // 推入缩放命令栈，支持撤销重做
    const scaleCmd = new SceneScaleCommand(this, oldScale, oldOffset, this.scale, this.offset);
    this.pushCommand(scaleCmd);

    this._updateVisibleNodes();
    this.render();
    this.fire('scale-change', { scale: this.scale, offset: this.offset });
  }
  ...
  // 碰撞检测相关
  /**
   * 获取鼠标转换后的画布坐标（抵消scale/offset）
   */
  _getCanvasMousePos(event) {
